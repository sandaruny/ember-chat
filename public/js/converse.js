      ...
      //    alert(msg);
        ...
        this.ChatBoxView = Backbone.View.extend({
            ...
            sendMessage: function (text) {
                ...
                var idSend = converse.connection.getUniqueId();
                var message = $msg({from: converse.connection.jid, to: bare_jid, type: 'chat', id: timestamp})
                    .c('body').t(text).up()
                    .c(ACTIVE, {'xmlns': Strophe.NS.CHATSTATES}).up();
                //    .c('markable', {'xmlns': Strophe.NS.CHATMARKER}).up();
                if (this.model.get('otr_status') != UNENCRYPTED) {
                    // OTR messages aren't carbon copied
                    message.c('private', {'xmlns': Strophe.NS.CARBONS});
                }
                // if(text == '/read'){
                //   message.c('markable', {'xmlns': Strophe.NS.CHATMARKER}).up();
                // }

              // var conn = new Strophe.Connection('http://openfire.mfsnet.io:7070/http-bind/ ');
                // conn.reciepts.send)()
                // alert('send by converse chatbox view');
                  converse.connection.receipts.sendMessage(message);
            //     alert(converse.connection.receipts.getUnreceivedMsgs());
                // converse.connection.send(message);
              ...
              if (converse.forward_messages) {
                    ...
                    var forwarded = $msg({to:converse.bare_jid, type:'chat', id:timestamp})
                                    ...
                                    .c('delay', {xmns:'urn:xmpp:delay',stamp:timestamp}).up()
                                    .cnode(message.tree());
                    converse.connection.receipts.sendMessage(forwarded);
                    // converse.connection.receipts.sendMessage(forwarded);
              ...
              //  this.appendMessage(message.c('body').t('/sent'));
            },
