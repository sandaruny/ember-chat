        ...
        if (allowed) {
            if (this.length > 0) {
                this.each(function (i, obj) {
                    ...
                    text = text.replace(/\(\^.\^\)b/g, '<span class="emoticon icon-thumbs-up"></span>');
		text = text.replace(/(BUY([\s])([^\s^&]*)([\s])([^\s^&]*))/, "<button onclick=\"alertSymbol(\'Buy\', \'$3\',\'$5\')\">Buy</button>");
		text = text.replace(/(SELL([\s])([^\s^&]*)([\s])([^\s^&]*))/, "<button onclick=\"alertSymbol(\'Sell\', \'$3\',\'$5\')\">Sell</button>");
		text = text.replace('ADVICE', '<iframe  src="send.html" height=130 frameborder="0" ></iframe>');
		text = text.replace('ADVICE2', '<br/><div height=200><h1>BUY or SELL</h1><button onClick=\"alert(\'done\')\">sell</button><button onClick=\"alert(\'done\')\">buy</button></div>');
                    text = text.replace('PIC', '<br/><img src=\"http://i.kinja-img.com/gawker-media/image/upload/jc1hxptjd4vu9qlgcxzh.jpg\" alt=\"Smiley face\" width=\"100\" height=\"100\">');
                    $(obj).html(text);
